...
Често множество от компоненти трябва да отразяват едни и същи тип данни. Препоръчваме тези данни да бъдат "издигнати" в state-а на близкият им общ родителски компонент. Нека видим пример за това.
...
Тези две функции преобразуват само числа. Ще напишем друга функция, която ще приема два аргумента, низа `temperature` и преобразувата функция, и като резултат функцията ще връща низ. Ще използваме тази функция, за да пресмятаме стойността на единият input, спрямо другият.

Също така ще връща празен низ при не валидна въведена `temperature`, и ще закръгля резултата до третия знак след запетайката:
...
## Издигане на State-а {#lifting-state-up}
...
Въпреки това, ние искаме двата да бъдат синхронизирани един с друг. Когато променяме стойносттана на input-а показващ температурата по Целций, input-а показващ температурата по Фаренхайт, трябва да показва преобразуваната температура, и обратното.

В React, споделянето на общ state, може да бъде постигнато като преместим state-а в най-близкият общ родителски компонент. Това наричаме "Издигане на State-а". Ще премахнем локалния state от `TemperatureInput` и ще го преместим в `Calculator`.

Ако `Calculator` притежава общият state, той вече е единствения източник на информация за температурата и за двата `TemperatureInput` компоненета. Така `Calculator` може да ги инструктира да имат консистентни стойности един спрямо друг. Тъй като props и на двата `TemperatureInput` компонента са контролирани от един и същи родителски компонент `Calculator`, двата input-а винаги ще бъдат синхронизирани един с друг.
...
Преди да почне да променяме `Calculator`, нека накратко да резюмираме промените по компонента `TemperatureInput`. Премахнахме локалният му state, и вместо `this.state.temperature` използва `this.props.temperature`. Също така вместо да извиква `this.setState()`, когато трябва да бъде направена промяна, ще извика `this.props.onTemperatureChange()`, който се подава от `Calculator`:
...
Ще съхраняваме `temperature` и `scale` на input-ите в локалния state на `Calculator`. Това е state-а който "издигнахме" от двата `TemperatureInput`, и това ще бъде единствения източник на информация и за двата. Това е мининума информация, от която се нуждаем за да ги рендерираме.
...
* React извиква render метода на `Calculator`, така разбира как трябва изглежда потребителският интерфейс. Стойностите на двата input-а се пресмятат на набаза на наличните температурата и температурната скала. Тук се случва и преобразуването на температурата от едната скала в другата.
...
* React актуализира DOM дървото в съответствие със въведениете стойностите. Последно промененият input ще получи стойността си и другият input ще получи новата си стойност след преобразуването на температура.

След всяка промяна преминава през тези стъпки, така че input-ите ще са винаги синхронизирани.

## Заключение {#lessons-learned}

Във всяка React аппликация, за определен тип данни които могат да бъдат променяни, трябва да има само един "източник на информация". Обикновенно, state се добава първо в компонентите, които имат нужда от него за да се рендерират. После ако и други компоненти се нуждаят от същия state, той може да бъде "издигнат" в най-близкият общ родителски компонент. Вместо да се опитваме да синхронизираме state-а на различни компоненти, би трябвало да разчитаме на [потокът на данни е от родител към деца](/docs/state-and-lifecycle.html#the-data-flows-down).

За да бъде "издигнат" даден state често това изисква повече код, спрямо подхода с друпосочен поток на данните, но като ползи получаваме, по-лесно изолиране и намиране на грешки в кода. Тъй като всеки state "живее" в конкретент компонент и само този компонент може да го променя, по този начин местата в кода в които може да има грешки свързани със съответния state значително намаля. Също така може да бъде имплементирана всякаква логика, която да модифицира входните данни от потребителя.
